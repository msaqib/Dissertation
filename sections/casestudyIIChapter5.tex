...
An impact of workload relocation is that some MS calls would be handed off to base stations from which they receive signals of relatively weaker strength. Even though the received signal strength is within the limits specified by the GSM standard, the effects of fading would become more pronounced. Consequently, the bit error rate on the MS would increase. Another effect on the system would be that the MS transmit power would need to be increased. These effects would result in quicker draining of MS batteries, possible increase in call drops and an impact on the call voice quality. This may result in customer churn which is undesirable. Our work explores the extent of possible electricity savings while ignoring these phenomena. Determination of the depletion in energy savings resulting from the above effects is left as future work. 

In the present work, we have assumed that there is no diversity in electricity prices in the cellular network due to the relatively short distance between neighboring sites. However, this may not be true when some of the sites run on diesel generators due to grid power outages. Due to a significant gap in demand and supply of electricity in developing countries, rolling blackouts are conducted during which some of the base stations must be powered by diesel generators. The unit price of electricity generated by backup diesel generators varies with the price of diesel fuel. A couple of years ago, in Pakistan, the diesel generated electricity was almost an order of magnitude more expensive than grid electricity. However, now, due to drop in petroleum product prices, diesel-generated electricity is cheaper than grid electricity. 

In a setting where there is electricity price diversity is evident in a cellular network, benefits of resource pruning can certainly be achieved. But can we benefit from workload relocation? Handling of calls at cheaper electricity price requires that base stations in close vicinity have different electricity prices. A situation where this is likely is two neighboring sites that are powered by different grid feeders. It appears that such diversity will be applicable to a small subset of base stations only. However, exploration of the additional benefits from this diversity in electricity prices is left as future work. 
